...
export default function AIInsights() {
    ...
    // Lazy-load investment data when that tab is first opened
    useEffect(() => {
    ...
    const insights = useMemo(() => {
        const budgetInsights = budgets.map(b => {
            ...
            return { ...b, pct, risk, dailySpent, projected, projectedPct, projectedRisk, recommendedDaily };
        });
    ...
    return (
        <div className="page">
            ...
            <div className="inv-tabs">
                <button
                    className={`inv-tab ${tab === 'budget' ? 'inv-tab-active' : ''}`}
                    onClick={() => setTab('budget')}
                >
                    <FiShield /> Budget Analysis
                </button>
                <button
                    className={`inv-tab ${tab === 'investment' ? 'inv-tab-active' : ''}`}
                    onClick={() => setTab('investment')}
                >
                    <FiTrendingUp /> Investment Analysis
                </button>
                <button
                    className={`inv-tab ${tab === 'chat' ? 'inv-tab-active' : ''}`}
                    onClick={() => setTab('chat')}
                >
                    <FiMessageSquare /> AI Chat
                    <span className="inv-tab-new-badge">NEW</span>
                </button>
            </div>
                ...
                <>
                    ...
                    <div className="ai-overview-grid">
                        <div className="ai-overview-card">
                            <div className="ai-ov-icon" style={{ color: overallCfg.color, background: overallCfg.bg }}>
                                <FiShield />
                            </div>
                            <div>
                        ...
                        <div className="ai-overview-card">
                            <div className="ai-ov-icon" style={{ color: '#6366f1', background: 'rgba(99,102,241,0.08)' }}>
                                <FiTarget />
                            </div>
                            <div>
                        ...
                        <div className="ai-overview-card">
                            <div className="ai-ov-icon" style={{ color: '#10b981', background: 'rgba(16,185,129,0.08)' }}>
                                <FiDollarSign />
                            </div>
                            <div>
                                <p className="ai-ov-label">Daily Limit</p>
                                <p className="ai-ov-value">{fmt(overallDailyLimit)}<span className="ai-ov-sub">/day</span></p>
                            </div>
                        </div>
                        <div className="ai-overview-card">
                            <div className="ai-ov-icon" style={{ color: '#f59e0b', background: 'rgba(245,158,11,0.08)' }}>
                                <FiTrendingDown />
                            </div>
                            <div>
                    ...
                    {alerts.length > 0 && (
                        <div className="ai-section">
                            <h3 className="ai-section-title"><FiAlertTriangle /> Overspend Alerts</h3>
                            <div className="ai-alerts">
                                {alerts.map(b => {
                                    const cfg = RISK_CONFIG[b.projectedRisk];
                                    return (
                                        <div key={b.id} className="ai-alert-item" style={{ borderColor: cfg.color }}>
                                            <span style={{ color: cfg.color }}>{cfg.icon}</span>
                                            <div>
                                                <strong>{b.categoryName}</strong>
                                                <span className="ai-dim"> — projected {fmt(b.projected)} ({b.projectedPct.toFixed(0)}% of budget)</span>
                                            </div>
                                        </div>
                                    );
                                })}
                    ...
                    {/* ── Budget Cards ── */}
                    <div className="ai-section">
                        <h3 className="ai-section-title"><FiShield /> Budget Breakdown</h3>
                        ...
                        <div className="ai-section">
                            ...
                            <div className="ai-cat-grid">
                                {categorySpending.map(c => {
                                    ...
                                    return (
                                        <div key={c.name} className="ai-cat-premium">
